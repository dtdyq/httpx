...
import org.dyq.httpx.route.util.MutableCharArray;
...
public class Router<T> {
    private static final Logger log = LoggerFactory.getLogger("RouteKit");
    ...
    public Match<T> routeOrNull(CharArray input) {
        ...
        if (match != null) {
            log.info("Routing `%s`: return immediately from the quick-match index".formatted(input));
    ...
    private static <T> Match<T> navigate(CharArray input, Node<T> current) {
        ...
        while (array.isNotEmpty()) {
            ...
            if (maxNode == null) {
                log.info("Routing `%s`: no continuation found at `%s`".formatted(input, array));
        ...
        if (!current.isTerminal()) {
            log.info("Routing `%s`: matches non-terminal node (middle of the rule)".formatted(input));
            return null;  // matches part of the rule
        }
        log.info("Routing `%s`: matches with variables %s".formatted(input, vars));
